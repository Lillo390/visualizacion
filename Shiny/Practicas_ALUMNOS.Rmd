...
ui<-navbarPage(theme = shinytheme("slate"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      p("Aquí mostraremos las variables")
    )
  )
)
...
ui <- fluidPage(titlePanel("Fechas"),
      ...
      dateInput("una_fecha", "Una fecha", value = "2016-02-17", min = NULL, max = NULL, format = "yyyy-mm-dd", language = "en"),
      dateRangeInput("rango_fechas", "Rango de fechas", start = "01-01-2016", end =NULL, format = "dd-mm-yyyy", separator= "a",language = "es",weekstart= 1)
                    )
server <- function(input, output) { }
...
ui<-navbarPage(theme = shinytheme("slate"),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput("SelectMachine", "Selecciona máquina", 
                                 c(Datos$matricula))
    ),
    ...
    navbarMenu("Estado de la máquina",
             tabPanel("Evolución temporal alarmas",sidebarPanel(
      p("Alarmas RadioButtons"), 
      radioButtons("radiobuttons1", "Selecciona la franja horaria", 
                                 c(colnames(Datos)[grep("^a", colnames(Datos))]))
    )), 
             tabPanel("Registros de la máquina",sidebarPanel(
               p("Alarmas checkbox"),
               checkboxGroupInput("CheckBoxAlarm", "Selecciona las alarmas para ver en la tabla", 
                                         choices=c(colnames(Datos)[grep("^a", colnames(Datos))] ))
  ...
  tabPanel("Estadísticos Globales Temporales",
           sidebarLayout(
             sidebarPanel(p("PERIODO Y ESTADÍSTICOS"),
              ...
              selectInput("SelectAlarm", "Selecciona alarma", 
                                 c(colnames(Datos)[grep("^a", colnames(Datos))] )),
              
              sliderInput("slider_bins", label = h3("Selecciona el número de bins"), 
...
ui<-fluidPage(titlePanel("Primer gráfico!"),
    sidebarLayout(
      sidebarPanel(
        selectInput("cilindros", "Nº de cilindros",c(unique(mtcars$cyl)))
      ),
      mainPanel(
        plotOutput("grafico")
      )
  ...
  server<-function(input, output) {
    output$grafico <- renderPlot({
      df = mtcars[mtcars["cyl"]==input$cilindros,]
      ggplot(df, aes(disp, mpg, size=hp, col=wt)) +
  geom_point()
      })
...
server <- function(input, output) {
    ...
    observe({if (counter$countervalue > 10 | counter$countervalue < -10){showModal(modalDialog("Max count reached!"))
...
library(dplyr)
df_filtered <- DF %>%
  filter(STATION == "Valencia" & substr(DATE, 1, 4) == "2008")

ggplot(df_filtered, aes(x = DATE, y = TMAX)) +
  geom_line() +
  geom_smooth() +
  labs(x = "Fecha", y = "Temperatura (°C)", title = "Evolución de la temperatura máxima en Valencia en el año 2008")
```
...
ui<-fluidPage(
  ...
  titlePanel("PRECIPITACIONES"),
    ...
    wellPanel(fluidRow(column(4,radioButtons("Estacion", "Selecciona la estación", c(unique(DF$STATION)))),
      ...
      column(4,sliderInput("Year", "Selecciona un año",  
                  min = min(lubridate::year(DF$DATE)), max = max(lubridate::year(DF$DATE)), value = min(lubridate::year(DF$DATE))))))
    ,
...
server <- function(input, output) {
  ...
  datos <- reactive({df_filtered <- DF %>%
  filter(STATION == input$Estacion & substr(DATE, 1, 4) == input$Year)})
  
  output$TMAX <- renderPlot({
ggplot(datos(), aes(x = DATE, y = TMAX)) +
  geom_line() +
  geom_smooth() +
  labs(x = "Fecha", y = "Temperatura (°C)", title = "Evolución de la temperatura máxima")
  })
...
ggplot(datos(), aes(x = DATE, y = TAVG)) +
  geom_line() +
  geom_smooth() +
  labs(x = "Fecha", y = "Temperatura (°C)", title = "Evolución de la temperatura media")
  })
...
ggplot(datos(), aes(x = DATE, y = TMIN)) +
  geom_line() +
  geom_smooth() +
  labs(x = "Fecha", y = "Temperatura (°C)", title = "Evolución de la temperatura mínima")
  })
}
shinyApp(ui, server)
...
server <- function(input, output) { 
  ...
  Tipos_R<-reactive({
                      ...
                      nombre_df=load(a$datapath) # # devuelve el nombre del dataframe que contiene el fichero
...
```{r, echo=FALSE}
x <- c(1, 2, 3, 0, 5, 6, 0, 8, 9)
join <- c(1, diff(x) != 0 & x[-length(x)] != 0)
plot(x, type = "p")
for (i in 2:length(x)) {
  if (join[i]) {
    lines(c(i-1, i), c(x[i-1], x[i]))
  }
}
```
...
# Datos de ejemplo
datos <- data.frame(x = 1:10, y1 = rnorm(10), y2 = rnorm(10), y3 = rnorm(10))

ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      radioButtons("var_y", "Selecciona variable Y", choices = c("y1", "y2", "y3"))
    ),
    mainPanel(
      plotOutput("grafica")
    )
...
server <- function(input, output) {
  ...
  hacer_grafica <- function(datos, var_y) {
    ggplot(datos, aes(x = x, y = !!sym(var_y))) +
      geom_line() +
      labs(x = "X", y = var_y, title = paste("Gráfica de", var_y))
  }
  
  # Renderización de la gráfica
  output$grafica <- renderPlot({
    hacer_grafica(datos, input$var_y)
  })
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      radioButtons(
        inputId = "y_variable",
        label = "Selecciona la variable Y",
        choices = c("y1", "y2")
      )
    ),
    mainPanel(
...
server <- function(input, output) {
  ...
  # Crear una variable reactiva para almacenar los datos originales
  datos_originales <- reactive({
    df
  })
  
  # Filtrar los datos según la selección del usuario y generar el gráfico
  output$grafico <- renderPlot({
    ggplot(datos_originales(), aes(x = x, y = .data[[input$y_variable]])) +
      geom_point()
  })
}
...
ui<-navbarPage(theme = shinytheme("slate"),
               ...
               navbarMenu("Estado de la máquina",
                          tabPanel("Evolución temporal alarmas",sidebarPanel(
                            p("Alarmas RadioButtons"),uiOutput("AlarmRadioButtons")
                          ),mainPanel(
                            plotOutput("AlarmEv")
                          )
                          ), 
                          tabPanel("Registros de la máquina",sidebarPanel(
                            p("Alarmas checkbox"), uiOutput("AlarmCheckbox"),
                            checkboxGroupInput("CheckBoxAlarm", "Selecciona las alarmas para ver en la tabla", 
...
server <- function(input, output) { 
  ...
  datos <- reactive({ 
    data <- input$DatosFichero
    req(data)
    name <- load(data$datapath)
    datos <- eval(parse(text=name))
  ...
  output$table <- renderDataTable(datos()[c("matricula","dia",input$checkAl,"prob_orden")])
  ...
  output$AlarmEv <- renderPlot({
    ggplot(dplyr::filter(datos(),matricula == input$SelMach), aes_string(x = dia, y = input$SelAl)) +
      geom_line() +
      geom_point() +
